...
const SubcategoryCard = memo(({ subcategory, icon: Icon, isSelected, onClick }: { 
  ...
  const getSubcategoryImage = () => {
    switch(subcategory) {
      case "Home decor":
        return "/home_decor.png";
      case "Furnishing":
        return "/home_furnishing.png";
      case "Kitchen":
        return "/home_kitchen.png";
      case "Groceries":
        return "/home_groceries.png";
      case "Electronics":
        return "/home_electronics.png";
      case "Gadgets":
        return "/home_gadgets.png";
      case "Books":
        return "/home_books.png";
      default:
        return "/home_product_1.webp";
    }
  };
        ...
        >
          ...
          <Link href={`/product/${product.id || product.id}`}>
            <div className="relative aspect-[3/4] overflow-hidden bg-gray-100 flex-shrink-0">
              ...
              {/* In Cart Badge */}
              {isInCart && (
                <div className="absolute top-2 left-2 bg-amber-600 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-sm z-10 flex items-center gap-1">
                  <CheckCircle size={10} />
                  In Cart
                </div>
              )}
          ...
          <div className="p-2 sm:p-2.5 flex flex-col flex-1">
            <Link href={`/product/${product.id || product.id}`}>
              <h3 className="text-xs sm:text-sm font-medium text-gray-900 line-clamp-2 min-h-[2rem] sm:min-h-[2.2rem] mb-1 hover:text-amber-600 transition-colors">
            ...
            <div className="mt-auto">
              {isInCart ? (
                ...
                <button
                  ...
                  disabled={localCartLoading || !user}
                  className="w-full bg-amber-600 hover:bg-amber-700 text-white py-1.5 rounded-md font-medium text-[10px] sm:text-xs transition-all duration-300 flex items-center justify-center gap-1 disabled:opacity-50"
                >
                  {localCartLoading ? (
                    <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
                  ) : (
        ...
        <button
          ...
          className={`flex items-center justify-center w-7 h-7 sm:w-8 sm:h-8 rounded-md text-xs ${
            currentPage === 1 || loading
              ...
              : "bg-white text-gray-600 hover:bg-amber-600 hover:text-white shadow-sm"
...
// Main HomeCollection Component
export default memo(function HomeCollection({ user }: HomeCollectionProps) {
  const dispatch = useAppDispatch();
  const [selectedSubcategory, setSelectedSubcategory] = useState<string | null>(null);

  const { homeProducts, homePagination, loading, error } = useAppSelector(
    (state) => ({
      homeProducts: state.categoryProducts.homeProducts,
      homePagination: state.categoryProducts.homePagination,
      loading: state.categoryProducts.loading.home,
      error: state.categoryProducts.error.home,
    }),
  );
  ...
  useEffect(() => {
    dispatch(
      fetchHomeCategoryProducts({
        page: homePagination.page,
        limit: homePagination.productsPerPage,
        subcategory: selectedSubcategory || undefined,
      }),
    );
  }, [dispatch, homePagination.page, homePagination.productsPerPage, selectedSubcategory]);
      ...
      <section className="w-full py-4 sm:py-6">
        <div className="text-center py-8">
          ...
          <button
            ...
            className="px-4 py-1.5 bg-amber-600 text-white text-sm rounded-md hover:bg-amber-700 transition-colors"
          >
            Try Again
          </button>
        </div>
  ...
  return (
    <section className="w-full py-3 sm:py-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
        <div>
          ...
          <p className="text-xs text-gray-500">
            {homePagination.totalProducts} products
      ...
      {homePagination.totalPages > 1 && (
        <Pagination
          currentPage={homePagination.page}
